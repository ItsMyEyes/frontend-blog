                    ...
                    <div class="collapse navbar-collapse" id="navbarNav">
                        <ul class="navbar-nav ml-auto">
                            <li class="nav-item active">
                                <router-link active-class="active" class="nav-link" to="/coming-soon-mobile" aria-disabled="true">Save draft</router-link>
                            </li>
...
export default {
    ...
    methods: {
        async getData() {
            const data = await axios.get(`posting/detail?id=${this.$route.params.id}`)
            data.data.dataPost.category_postingan.map(res => { 
                this.tagJson.push(res.id)
            })
            data.data.dataPost.category_postingan.map(res => { 
                const data = { id: res.id, title: res.title } 
                this.tags.push(data)
            })
            this.dataJson = data.data.data
            this.form.title = data.data.dataPost.title
            this.dataBackground = data.data.dataPost.thumbail_url
            this.form.thumbail_url = data.data.dataPost.thumbail_url
            this.loadEditor()
        },
        async getCategory() {
        ...
        async save() {
            ...
            $('#app').addClass('blur')
            axios.put(`posting/edit?id=${this.$route.params.id}`,data)
            .then(data => {
                console.log(data)
                const category = {
                    post_id: data.data.data,
